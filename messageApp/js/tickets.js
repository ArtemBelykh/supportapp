        ...
        <div class="jumbotron form-create-message">
          <form>
            <div class="container">
              ...
              <br />
              <div class="form-create-message__footer btn-group-container d-flex">
                ${generateBtn('Submit', undefined, 'sendNewMessage')}
                ${generateBtn('Cancel', undefined, 'CancelNewMessage', '', 'form-create-message__remove btn-danger cancel_btn')}
                <style>
					.cancel_btn {
						margin-left: 10px;
					}
...
`
  ...
  function generateNoteBodies(data) {
    ...
    $('.tab-pane.active')
      .append(`<div class="note-body" style="width: 100%;margin-top: 0;">`)
      .append(`<div class="note-body__container"></div>`);

    $('.note-body__container').append(`
<div class="tickets__subj_container" style="display: flex;align-items: center;">
    <button type="button" class="btn btn-primary note-body__bnt-back"><i class="glyphicon glyphicon-arrow-left"></i> Back</button> 
    <h3 class="note-body__title" style="margin-top: 15px;">${window.innerWidth <= '1023' ? data.notes.subject.substring(0, data.notes.subject.substring(0,25).lastIndexOf(" ")) + " ..." : data.notes.subject}</h3>
    <select onchange="window.location.href = '/productApp/tickets.php'" id="statuses" class="get_ticket_status" style="width: 150px;height: 40px;font-family: 'Book Antiqua'; font-size: 16px;letter-spacing: 1.5px; position: absolute;right: 30px;"></select>
</div>
<div class="tickets__tags"></div>`);
    if (data.status_id === 1) {
        $('#statuses').append(`<option value="1" style="font-family: 'Book Antiqua'; font-size: 16px;letter-spacing: 1.5px;" selected>Opened</option>`)
        $('#statuses').append(`<option value="2" style="font-family: 'Book Antiqua'; font-size: 16px;letter-spacing: 1.5px;">Pinned</option>`)
        $('#statuses').append(`<option value="3" style="font-family: 'Book Antiqua'; font-size: 16px;letter-spacing: 1.5px;">Closed</option>`)
    }else if(data.status_id === 2) {
        $('#statuses').append(`<option value="1" style="font-family: 'Book Antiqua'; font-size: 16px;letter-spacing: 1.5px;">Opened</option>`)
        $('#statuses').append(`<option value="2" style="font-family: 'Book Antiqua'; font-size: 16px;letter-spacing: 1.5px;" selected>Pinned</option>`)
        $('#statuses').append(`<option value="3" style="font-family: 'Book Antiqua'; font-size: 16px;letter-spacing: 1.5px;">Closed</option>`)
    }else if(data.status_id === 3) {
        $('#statuses').append(`<option value="1" style="font-family: 'Book Antiqua'; font-size: 16px;letter-spacing: 1.5px;">Opened</option>`)
        $('#statuses').append(`<option value="2" style="font-family: 'Book Antiqua'; font-size: 16px;letter-spacing: 1.5px;">Pinned</option>`)
        $('#statuses').append(`<option value="3" style="font-family: 'Book Antiqua'; font-size: 16px;letter-spacing: 1.5px;" selected>Closed</option>`)
    }

      $('.get_ticket_status').on('click', (event) => {
      event.preventDefault()
      const ticket_status = $('.get_ticket_status').val()
      console.log(ticket_status)
      $.ajax({
        type: "GET",
        url: `${apiUrl}sql_update_support_ticket_status.php`,
        data: {
    ...
    $('.tickets__tags').after('<div class="note-body__tags" style="display:flex;flex-wrap: wrap;"></div>');
    data.tags?.forEach(tag => {
      $('.note-body__tags').append(`<div class="note-body__tag-item" style="display: flex; flex-wrap: wrap">${tag.tag_name}</div>`)
    })
  ...
  function generateBtn(title, glyphIcon, id, type = 'button', ...classes) {
    ...
    return `<button type="${type}" class="btn btn-primary btn-s ${classes.join(' ')}" href="#" role="button" id="${id}">${title}</button>`;
  }
  ...
  function sendNewMessageHandler(e, msgData) {
    const container = $('.form-create-message__input-wrapper .modal-recipients-result__item');
      ...
      fetch(`${apiUrl}sql_create_ticket.php?contact_id=${contactId}`)
        ...
        .then(d => {
          ...
          fetch(`${apiUrl}sql_update_support_ticket_status.php?status_id=${status}&ticket_id=${ticketId}`)
            ...
            .then(d => d)])
          .then(d => {
            // Promise.all(tagIds.length > 0 ? 
            //   tagIds.map(el => fetch(`${apiUrl}sql_insert_ticket_tag.php?tag_id=${el}&ticket_id=${ticketId}`)
            //   .then(r => r.json())
            //   .then(d => d)
            // ): [])
            Promise.all( 
                  ...
                  if (toEmployeeIds.length) {
                    Promise.all(toEmployeeIds.map(id => sendId(id, noteBodyId)))
                      .then(r => {
                        ...
                        window.location.href = '/productApp/tickets.php'
    ...
    if (noteBody.val()) {
      fetch(`${apiUrl}sql_create_ticket.php?contact_id=${contactId}`)
        ...
        .then(d => {
          ...
          fetch(`${apiUrl}sql_update_support_ticket_status.php?status_id=${sta}&ticket_id=${ticketId}`),
          fetch(`${apiUrl}sql_update_support_ticket_status.php?status_id=${status}&ticket_id=${ticketId}`)
            .then(r => r.json())
            .then(d => d)])
          .then(d => {
