		...
		if (!$(`#tblMain-${id} thead`).length) {
			...
			<thead>
				<tr>
					...
					<th>Subject</th>
					<th>Date</th>
					${id !== 'archived' ? '<th>Add to archive</th>' : '<th></th>'}
				</tr>
			</thead>
		`)
		}
		...
		table = await $(`#tblMain-${id}`).DataTable({
			...
			columns: [
				...
				},
				{
					...
					render: () => {
						if (id !== 'archived') {
							return '<svg aria-hidden="true" style="width: 15px; cursor: pointer" focusable="false" data-prefix="fas" data-icon="file-archive" class="svg-inline--fa fa-file-archive fa-w-12" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512"><path fill="currentColor" d="M377 105L279.1 7c-4.5-4.5-10.6-7-17-7H256v128h128v-6.1c0-6.3-2.5-12.4-7-16.9zM128.4 336c-17.9 0-32.4 12.1-32.4 27 0 15 14.6 27 32.5 27s32.4-12.1 32.4-27-14.6-27-32.5-27zM224 136V0h-63.6v32h-32V0H24C10.7 0 0 10.7 0 24v464c0 13.3 10.7 24 24 24h336c13.3 0 24-10.7 24-24V160H248c-13.2 0-24-10.8-24-24zM95.9 32h32v32h-32zm32.3 384c-33.2 0-58-30.4-51.4-62.9L96.4 256v-32h32v-32h-32v-32h32v-32h-32V96h32V64h32v32h-32v32h32v32h-32v32h32v32h-32v32h22.1c5.7 0 10.7 4.1 11.8 9.7l17.3 87.7c6.4 32.4-18.4 62.6-51.4 62.6z"></path></svg>'
						}
						return '';
					}
				}
			...
			columnDefs: [
				...
				{
					targets: -1,
					className: `${id !== 'archived' ? 'dt-body-right add-to-archive' : ''}`
				}
			...
			initComplete: function (oSettings, json) {
				if (cb) cb();
				$('.tab-pane.active .table .add-to-archive').on('click', e => {
					$('#tblMain-inbox_wrapper').hide();
					showSpinner();
					var data = $(e.currentTarget).closest('table').DataTable().row(e.currenttarget).data();
					console.log(data)
					addToArchieve(data.id)
				})
...
<div class="tickets__subj_container" style="display: flex;align-items: center;">
    ...
    <h3 class="note-body__title" style="margin-top: 30px;">${data.subject}</h3>
    <svg aria-hidden="true" style="width: 15px; cursor: pointer; position: absolute;right: 30px;" focusable="false" data-prefix="fas" data-icon="file-archive" class="svg-inline--fa fa-file-archive fa-w-12 add_archived note-body__bnt-back" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512"><path fill="currentColor" d="M377 105L279.1 7c-4.5-4.5-10.6-7-17-7H256v128h128v-6.1c0-6.3-2.5-12.4-7-16.9zM128.4 336c-17.9 0-32.4 12.1-32.4 27 0 15 14.6 27 32.5 27s32.4-12.1 32.4-27-14.6-27-32.5-27zM224 136V0h-63.6v32h-32V0H24C10.7 0 0 10.7 0 24v464c0 13.3 10.7 24 24 24h336c13.3 0 24-10.7 24-24V160H248c-13.2 0-24-10.8-24-24zM95.9 32h32v32h-32zm32.3 384c-33.2 0-58-30.4-51.4-62.9L96.4 256v-32h32v-32h-32v-32h32v-32h-32V96h32V64h32v32h-32v32h32v32h-32v32h32v32h-32v32h22.1c5.7 0 10.7 4.1 11.8 9.7l17.3 87.7c6.4 32.4-18.4 62.6-51.4 62.6z"></path></svg>
</div>`);
		data.note_bodies.forEach((element, i) => {
			...
			$('.add_archived').on('click', (event) => {
				event.preventDefault()

				console.log(data)

				fetch(`${apiUrl}sql_update_recipient_note_to_archived.php?note_id=${data.id}`)
					.then(r => r.json())
					.then(d => {
						console.log(d)
					})
	...
	function formOpenHandler(e, data = {}, purpose = 'new') {
		...
		$('#sendNewMessage').click(e => sendNewMessageHandler(e, data));
	...
	function sendNewMessageHandler(e, msgData) {
		...
		const container = $('.form-create-message__input-wrapper .modal-recipients-result__item');
		const subject = $('#subject').val() || msgData.subject;
					...
					if (d.message !== 'error') {
						...
						Promise.all(toEmployeeIds.map(id => sendId(id, noteBodyId)))
							.then(r => {
								...
								window.location.href = '/productApp/notifications.php'
							})
	...
	function addToArchieve(id) {
		fetch(`${apiUrl}sql_update_recipient_note_to_archived.php?note_id=${id}`)
			.then(r => r.json())
			.then(d => {
				hideSpinner();
				$('#tblMain-inbox_wrapper').show();
				loadTable('inbox');
	...
	// Spinner
	function showSpinner() {
		$('.main .tab-content').append('<div class="loader loader--bg loader--white"></div>');
	}
