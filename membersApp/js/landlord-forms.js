(() => {
  ...
  function showSpinner() {
    $('.main .landlord-forms-section .row')
      .find('.wrapper')
  ...
  getData(`${apiUrl}sql_get_land_files_by_contact_id.php`)
    .then(data => {
        ...
        .append(
          `<div class="row header-block header-block--wider">
            <div class="header-block__container">
              <h3 class="header-block__title">Landlord Forms</h3>
            </div>
          </div>
          <br />
          `
      ...
      const esaFileId = $('.landlordForms li').eq(0).data('id');
      if (esaFileId) {
      ...
      $('.landlordForms .panel').click(function (e) {
        const icon = $(this).find('i');
        $(this).find('.collapse').collapse('toggle');
        icon.toggleClass('--rotated');
      });

      $('.landlordForms__select').on('click', function (e) {
        e.stopPropagation();
      })
  ...
  function generateCollapseForLetters(data) {
    ...
    const container = $('.landlord-forms-section .row .wrapper');
    container.append(`
			<div class="row">
				<div class="wrapper">
        	<div class="landlordForms">
        ...
        return `
          <li data-id="${key}" class="panel panel-default">
            <div class="collapsible-header panel-heading">
              <div class="landlordForms__row">
								<i class="material-icons">expand_more</i>
								<span>${key}</span>
							</div>
							<div class="landlordForms__select">
  ...
  function sendLetterHandler(e, data, currEl) {
    ...
    e.stopPropagation();
    fetch(`${apiUrl}kla_send_landlord_forms.php`, { method: 'POST', body: JSON.stringify(data) })
      .then(r => r.json())
      .then(d => {
        if (d.result === '1') {
  ...
  function generateTextArea(parent, data) {
    parent.find('.landlordFroms__textarea').remove();
    return `<textarea class="landlordFroms__textarea">${data}</textarea>`
